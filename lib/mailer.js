...
module.exports = function (context) {
  ...
  function getTemplateOptions (job, state, type) {
    ...
    var project = job.project
      ...
      , triggerMessage = job.trigger.message
      , subject = '[STRIDER] - ' + display_name + ' (' + triggerType + ') test '
        + state + ' - ' + job._id.toString().substr(0,8)
  ...
  function sendToUniversalAddress(job, htmlTemplate, plainTextTemplate, state, callback) {
    var project = job.project
    ...
    mailer.send(process.env.EMAIL_NOTIFIER_ADDRESS, htmlOptions.subject, body_text, body_html, false, callback)
  }
  ...
  function sendSuccess(job, callback) {
    if (!_.isEmpty(process.env.EMAIL_NOTIFIER_ADDRESS)) {
      sendToUniversalAddress(job, test_succeed_html, test_succeed_plaintext, 'success', callback)
    } else {
  ...
  function sendFailure(job, callback) {
    if (!_.isEmpty(process.env.EMAIL_NOTIFIER_ADDRESS)) {
      sendToUniversalAddress(job, test_fail_html, test_fail_plaintext, 'failure', callback)
    } else {
